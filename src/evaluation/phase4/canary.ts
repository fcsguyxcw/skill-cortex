/**
 * Phase 4 — project-local shadow_replay validation harness（不真实宿主部署、不写用户环境）。
 *
 * leader D3：保留 canary 文件名与函数名以减小面，但本 harness 是 shadow_replay 验证方法
 * （ADR-0012：shadow replay 是验证方法，不是 procedure 状态），**不是 canary 发布**：
 * - procedure 始终为 validated，绝不执行 validated→canary→active 状态转换；
 * - 执行上下文恒为 executionContext="shadow_replay"（canary/active 上下文不放行 validated）；
 ...
 *   detector 判 abstain ⇒ artifact disposition=abstained ⇒ executor 统一产生
 *   procedure_abstained 回退慢路径（外层不再手工路由，验收修复）。
 *
  ...
  for (const case_ of HELDOUT_CASES) {
    const outcome = await execute({
      ...
      environment: {
        ...
        executionContext: CANARY_EXECUTION_CONTEXT,
        preconditions: [
